{ ... }: {
  flake.nixosModules.mainHardware = { config, lib, modulesPath, ... }: {
    ...
    swapDevices = [{
      device = "/swapfile"; # Path to the swap file
      size = 32 * 1024;         # Size in MiB (e.g., 16 GB)
    }];

    # Enables DHCP on each ethernet and wireless interface. In case of scripted networking
    # (the default) this is the recommended approach. When using systemd-networkd it'sgrub
